...
task 'build:server', 'build express server file from source files', (options) ->
	exec 'iced -c -o . src/server/', (err, stdout, stderr) ->
		err && throw err
		log 'Build Server OK!', 'green'
...
task 'build:coffee', 'build src/client.js file from source files', (options) ->
	...
	finish = ->
		...
		fs.writeFile 'src/client/coffee/client.coffee', content.join('\n\n'), 'utf8', (err) ->
			err && throw err
			exec 'iced -c -o www/js src/client/coffee/client.coffee', (err, stdout, stderr) ->
				err && throw err
...
task 'ship', 'commit to git and push to remote server', (options) ->
	...
	exec 'git add -f *;', (err, stdout, stderr) ->
		...
		exec 'rsync -avzP --exclude .git --exclude node_modules . ' + remote, (err, stdout, stderr) ->
			log stdout + stderr, 'light_grey'
			err && throw err
			exec 'git commit -am "' + message + '"; git push origin master', (err, stdout, stderr) ->
				log stdout + stderr, 'light_grey'
				err && log err, 'red'
				log 'Ship OK!', 'green'
